(function(global, factory) {
	...
	var R = function(conf) {
		...
		self.init = conf.init
		self.initPath = self.hostname + (conf.init || '')
		return self
	}
	...
	R.prototype = {
			...
			go: function(condition) {
				location.hash = this.hostname + condition
			}
		}
		// static
	R.stage = {}
	...
	R.run = function(instance) {
		if (!R.bindStatus) {
			...
			if (!instance.init) {
				if (!R.stage[instance.initPath]) {
					instance.when('', function() {}, {
						destroy: function() {}
					})
				}
			}
			location.hash = instance.initPath
		}
		R.checkQueue()
	}
	return R
